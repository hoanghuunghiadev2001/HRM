...
export default function ProposalDetailPage() {
  ...
  const confirmAction = async (action: "sign" | "approve", status: "approved" | "rejected") => {
    setActionLoading(true)
    try {
      await axios.post(`/api/proposals/${proposalId}/${action}`, { proposalId, status })
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case "pending_signatures": return { color: "orange", text: "Đang chờ ký", icon: <ClockCircleOutlined /> }
      case "waiting_approval": return { color: "blue", text: "Đang chờ duyệt", icon: <ClockCircleOutlined /> }
      case "approved": return { color: "green", text: "Đã duyệt", icon: <CheckCircleOutlined /> }
      case "rejected": return { color: "red", text: "Đã từ chối", icon: <CloseOutlined /> }
      default: return { color: "default", text: status, icon: <InfoCircleOutlined /> }
    }
  }
  ...
  const getTimelineItems = () => {
    ...
    proposal.signers.forEach((s) => {
      if (s.status !== "pending") items.push({
    ...
    proposal.approvers.forEach((a) => {
      if (a.status !== "pending") items.push({
  ...
  const renderPersonCard = (person: any, role: "proposer" | "signer" | "approver", status?: string, actionDate?: string) => {
    const roleConfig = { proposer: "#1890ff", signer: "#722ed1", approver: "#52c41a" }
    const statusConfig = {
    ...
    return (
      <Card
        ...
        style={{
          marginBottom: 12,
          border: person.id === currentUserId ? `2px solid ${roleConfig[role]}` : undefined,
          boxShadow: person.id === currentUserId ? `0 0 10px ${roleConfig[role]}20` : undefined,
        }}
      >
        <Row gutter={16} align="middle">
          <Col flex="none">
            <Badge dot={person.id === currentUserId} color={roleConfig[role]}>
          ...
          <Col flex="auto">
            <div className="flex items-center gap-2 justify-between ">
              <Text strong style={{ fontSize: 16 }}>
                ...
                {person.id === currentUserId && <Tag color={roleConfig[role]} style={{ marginLeft: 8 }}>Bạn</Tag>}
              </Text>
            </div>
            {status && <Tag color={statusConfig[status as keyof typeof statusConfig]?.color}>
              {statusConfig[status as keyof typeof statusConfig]?.icon}
              <span style={{ marginLeft: 4 }}>{statusConfig[status as keyof typeof statusConfig]?.text}</span>
            </Tag>}
            {actionDate && <div><Text type="secondary" style={{ fontSize: 11 }}>{new Date(actionDate).toLocaleString("vi-VN")}</Text></div>}
          </Col>
        </Row>
      </Card>
    )
    ...
    <div style={{ padding: 24, maxWidth: 1400, margin: "0 auto", backgroundColor: "#f5f5f5", minHeight: "100vh" }}>
        ...
        <Row gutter={24} align="middle">
          <Col flex="auto">
            <Space direction="vertical" size="small">
              <Title level={2}><FileTextOutlined /> {proposal.name}</Title>
              <Space>
                <Tag color={statusConfigFinal.color}>{statusConfigFinal.icon} <span style={{ marginLeft: 4 }}>{statusConfigFinal.text}</span></Tag>
      ...
      <Card title={<><InfoCircleOutlined /> Thông tin chi tiết</>} style={{ marginBottom: 24 }}>
        <Descriptions column={1} bordered>
          <Descriptions.Item label="Tên đề xuất">{proposal.name}</Descriptions.Item>
          ...
          <Descriptions.Item label="Trạng thái"><Tag color={statusConfigFinal.color}>{statusConfigFinal.icon} {statusConfigFinal.text}</Tag></Descriptions.Item>
      ...
      {/* Preview file */}
      {currentPreviewUrl && (
        <Card title={<><FolderViewOutlined /> Xem trước File: {proposal.file?.filename}</>} style={{ marginBottom: 24 }}>
