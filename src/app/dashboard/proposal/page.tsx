...
export default function ProposalCreator() {
  ...
  // ---------- new: proposal type ----------
  const [proposalType, setProposalType] = useState<"general" | "vehicle">("general")
  ...
  const generateDocxFromTemplate = async (templatePath: string, data: Record<string, any>): Promise<File> => {
    try {
      ...
      const filename = (data?.proposalName ? `${data.proposalName}` : "vehicle_proposal") + ".docx"
  ...
  const handleSubmit = async () => {
    ...
    if (proposalType === "general") {
      ...
      if (!timestart || !timeend) return message.error("Vui lòng chọn thời gian bắt đầu & kết thúc")
    ...
    try {
      ...
      formData.append("proposerId", "1") // TODO: replace with real user id
      ...
      if (proposalType === "vehicle") {
        ...
        try {
          const generatedFile = await generateDocxFromTemplate("/templates/vehicle_proposal_template.docx", tplData)
      ...
      <Row gutter={24}>
        <Col xs={24} lg={12}>
          ...
          {proposalType === "vehicle" && (
            <Card title={<><CarOutlined /> Đề Xuất Xe</>} style={{ marginBottom: 24 }}>
            ...
            title={
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {proposalType === "general" && pdfPreviewUrl && currentPdfFile && (
                  <Space>
                    <Button type="text" icon={<EyeOutlined />} onClick={() => window.open(pdfPreviewUrl, "_blank")} disabled={submitting}>
                      Mở rộng
                    </Button>
            ...
            {proposalType === "general" && pdfPreviewUrl && currentPdfFile ? (
              ...
              <div style={{ padding: 24 }}>
                ...
                <div style={{ marginTop: 8 }}>
                  Đây là loại <Text strong>Đề xuất xe</Text>. File sẽ được tạo tự động từ mẫu Word và gửi lên server khi bạn bấm Tạo Đề Xuất.
                </div>
                <div style={{ marginTop: 12 }}>
                  <Text type="secondary">Bạn có thể tải hàng mẫu (DOCX) tại <a href="/templates/vehicle_proposal_template.docx" target="_blank" rel="noreferrer">/templates/vehicle_proposal_template.docx</a></Text>
                </div>
              </div>
            ) : (
              <div
